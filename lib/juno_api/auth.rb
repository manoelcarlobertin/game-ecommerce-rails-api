module JunoApi
  class Auth
    ...
    SECONDS_TO_WAIT_PROCESSING = 0.5 # A cada meio segundo vamos verificar se há algo processando.
    LIMIT_RATE_TO_RENEW = 90

    base_uri JUNO_AUTH_URL

# vamos deixar o *token* de acesso, o tempo de expiração e o horário da requisição disponíveis para acesso externo
...
#----------------------------------------------------------------
    ...
    def self.check_instance
      if @instance.blank? || is_about_to_expire?(@instance)
        @processing = true
        @instance = new # onde fazemos a requisição para a API da Juno
        @processing = false
      end
    end
